...
class PoseDetector {
  ...
  /// Call to native to create a ML Kit PoseDetector
  Future<String> startPoseDetector() async {
    String result = await PluginPose._channel
  ...
  Future<Map<int, PoseLandmark>> processImage(InputImage inputImage) async {
    ...
    // print("processing image in pose detector: ${inputImage._getImageData()}");
    List<dynamic> result = await PluginPose._channel
        .invokeMethod('handleDetection', <String, dynamic>{
  ...
  Future<void> close() async {
    if (!_isClosed && _isOpened) {
      await PluginPose._channel.invokeMethod('closePoseDetector');
      _isClosed = true;
      _isOpened = false;
    }
  }
...
class PoseLandmark {
  ...
  static const int NECK_BASE = 33;

  static const int PELVIS_CENTER = 34;

  static const int HEAD_TOP = 35;
